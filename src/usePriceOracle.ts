...
const ethUSDFetcher =
  ...
  async ([_, blockTag]) => {
    ...
    if (result["market_data"] != undefined && result["market_data"]["current_price"]["usd"] != undefined) {
      return Number(result["market_data"]["current_price"]["usd"]);
    }
    return undefined;
  };
...
export const useETHUSDOracle = (
  ...
  if (GLQPriceCache == undefined) {
    const fetcher = ethUSDFetcher(provider);
    const { data, error } = useSWRImmutable(ethUSDFetcherKey(blockTag), fetcher);
    if (error || data == undefined) {
      return undefined;
    }
    let cost:BigNumber = BigNumber.from((data * 100000000).toFixed(0));
    GLQPriceCache = cost;
  }
  return GLQPriceCache !== undefined ? BigNumber.from(GLQPriceCache) : BigNumber.from('1566000');
};
